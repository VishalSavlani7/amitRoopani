...
const Worksheets = () => {
  const [data, setData] = useState<SheetWorksheet[]>([]);
  ...
  const SHEET_ID = "1oLiSy7jnSTYE5KDCMMDp46Y_QmXNpzpaJEwWQCHrRiY";
  const API_KEY = "AIzaSyC1UAGjqkukOzZz1h2ShohCp62StRLc38s";
  // useEffect(() => {
  //   fetch("https://sheetdb.io/api/v1/l689mrz1rdn6e")
  //     .then((res) => res.json())
  //     .then((d: SheetWorksheet[]) => { setData(d); setLoading(false); })
  ...
  useEffect(() => {
    fetch(`https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/A1:Z1000?key=${API_KEY}`)
      ...
      .then((d) => {
        ...
        const parsed: SheetWorksheet[] = rows.map((row: string[]) => ({
          No:          row[0] ?? "",
          Board:       row[1] ?? "",
          class:       row[2] ?? "",
          subject:     row[3] ?? "",
    ...
    <Layout>
      <section className="page-section">
        <div className="container mx-auto">
          <div className="mb-10">
            <span className="text-accent font-body font-semibold text-sm tracking-widest uppercase">Practice</span>
            <h1 className="section-title mt-2">Worksheets</h1>
            <p className="section-subtitle">Follow the steps below to find your worksheets.</p>
          </div>
